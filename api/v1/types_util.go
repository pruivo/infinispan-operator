...
// GetCrossSiteRouteName computes the route service name
func GetCrossSiteRouteName(name, namespace string, customRouteHostname bool) string {
	...
	if len(externalName) > MaxRouteObjectNameLength {
		externalName = externalName[0 : MaxRouteObjectNameLength-1]
	...
	// (-2) comes from the '-' between namespace and name and we append 'b'
